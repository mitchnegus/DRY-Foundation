"""Helper objects to improve testing of application endpoints."""
...
# Provide a generic test case instantiation to use with pytest test classes
unit_test_case = unittest.TestCase()


class _ResponseSoup:
    """
    An object to interface between BeautifulSoup and a request response.
    """
    ...
    def _load_soup(self):
        ...
        if isinstance(self.html, list):
            soup = [BeautifulSoup(_, "html.parser") for _ in self.html]
...
class TestRoutes:
    """
    A pytest-style test class to serve as the base for testing routes.
    """
    ...
    def route_loader(method):
        ...
        def wrapper(self, route, *args, **kwargs):
            ...
            response = method(self, route, *args, **kwargs)
            self.active_response = _ResponseSoup(response)
            return response
    ...
    @property
    def html(self):
        return self.active_response.html if self.active_response else None

    @property
    def soup(self):
        return self.active_response.soup if self.active_response else None

    @route_loader
    def get_route(self, route, *args, **kwargs):
        """Load the HTML returned by accessing the route (via 'GET')."""
        return self.client.get(route, *args, **kwargs)
    ...
    def page_heading_includes_substring(self, substring, level="h1"):
        ...
        ----------
        substring : str
            A substring to find in the page heading.
        level : str
            The tag type to treat as the page heading. The default is
            ``h1``.
        ...
        """
        heading_tags = self.soup.find_all(level)
        return any(
            substring in string for tag in heading_tags for string in tag.strings
        )
