...
class AppTestManager:
    ...
    on two pytest fixtures (``app_context`` and
    ``app_transaction_context``) to control the scope of the two apps.
    The ``app_context`` fixture is created just once for the session and
    is then automatically used in all tests. On the other hand, the
    ``app_transaction_context`` fixture may be manually included in any
    test, which causes an ephemeral app to be created (and then used)
    only for that one test. To avoid cluttering test signatures, the
    ``transaction_lifetime`` decorator helper is provided to signal that
    a test should use the ephemeral app rather than calling the
    ``app_transaction_context`` fixture directly.
    ...
    **config_kwargs :
    ...
    def app_test_context(self, app_name, **kwargs):
        ...
        Given the app name (either ``"ephemeral_app"`` or
        ``"persistent_app"``), this context manager defines a context
        for that app, including the creation of a temporary database to
        be used by that version of the test app. Multiple test contexts
        may be generated and associated with the ``AppManager`` to
        enable access to different apps depending on the test.
