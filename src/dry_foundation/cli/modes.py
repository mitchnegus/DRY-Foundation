...
class CustomCLIAppMode(ABC):
    ...
    **kwargs :
    ...
    #
    #   Notes again
    #   The Flask application object can load a configuration. The DryFlask
    #   object customizes the Flask application object such that it falls
    #   back to using a default configuration type (generally the
    #   `DevelopmentConfig`) when not otherwise specified (such as when
    #   testing and a test config is passed directly). This application mode
    #   object will override that default configuration type with a mode-
    #   specific configuration type. The instance-based configurations used
    #   by this package rquire knowledge of the application's instance path,
    #   and so must be generated by the application object (after it has
    #   determined the instance-path).
    #
    #   Furthermore, because CLI invocations of the app appear to hardcode
    #   some calls to the `create_app` function—the application factory-
    #   without accepting arguments (at least as of Flask version 3.0.3),
    #   knowledge of the instance-path means that even using `partial` to
    #   pass a pre-instantiated configuration is less than ideal.

    # def __init__(self, context, config=None):
    def __init__(self, context, config=None):
        self._context = context
        DryFlask.set_default_config_type(self.config_type)
        # Set the app factory to pass a configuration based on the mode
        context.obj.create_app = partial(context.obj.create_app, config=config)
        self.application = context.obj.load_app()
        super().__init__()
